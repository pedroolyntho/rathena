...
int status_base_amotion_pc(map_session_data* sd, struct status_data* status)
{
	...
	amotion -= amotion * (20 * status->agi + 20 * status->dex) / 1000;
...
unsigned short status_base_atk(const struct block_list *bl, const struct status_data *status, int level)
{
	...
	switch(((TBL_PC*)bl)->status.weapon) {
		//case W_BOW: //custom habib, bow no longer gets atk from DEX but STR
		//case W_MUSICAL:
		//case W_WHIP:
		//case W_REVOLVER:
		//case W_RIFLE:
	...
	**/
	switch (bl->type) {
		...
		case BL_PC:
			...
			str = dstr + ((dstr * 10 + level * 10 / 4) / 10 + 5 * status->pow)/2;
			//str = 1 + (level / 10);
			//str += dstr / 5;//CUSTOM
#else
			dstr = str / 10;
...
unsigned short status_base_matk_min(struct block_list *bl, const struct status_data* status, int level)
{
	switch (bl->type) {
		...
		default:
			...
			return status->int_ + (status->int_ / 2) + (level / 4) + 5 * status->spl;
			//return ((status->int_ / 5) + 1 + (level / 10));//CUSTOM
	}
}
...
void status_calc_misc(struct block_list *bl, struct status_data *status, int level)
{
	...
	if (bl->type == BL_HOM) {
		...
		stat += 10 + (8 * level) + (5 * status->dex) + (bl->type == BL_PC ? status->luk / 1 + 175 : 150);; //CUSTOM HABIB
		stat += 2 * status->con;
		status->hit = cap_value(stat, 1, SHRT_MAX);
		// Flee
		stat = status->flee;
		...
		stat += (5 * level) + (5 * status->agi) + (bl->type == BL_MER ? 0 : bl->type == BL_PC ? status->luk / 1 : 0) + 100;//CUSTOM
		stat += 2 * status->con;
		status->flee = cap_value(stat, 1, SHRT_MAX);
		// Def2
		if (bl->type == BL_MER)
		...
		if (bl->type == BL_MER)
			...
			stat += (int)(bl->type == BL_PC ? ((float)level / 1) : ((float)(status->int_ + level) / 4));//CUSTOM
		}
		status->mdef2 = cap_value(stat, 0, SHRT_MAX);
		// PAtk
		stat = status->patk;
	...
	if( bl->type&battle_config.enable_critical ) {
		...
		stat += 10 + (status->luk*15); //CUSTOM every 1 luk = +1.5 crit
#else
	...
	if (bl->type&battle_config.enable_perfect_flee) {
		...
		stat += (status->luk*5) + 10; // Custom every 1 luk = +0.5 perfect flee
	...
	if (type == STATUS_BONUS_FIX) {
		...
		if (bl->type == BL_PC) {
			...
			if ((skill_lv = pc_checkskill(sd,WM_LESSON)) > 0)
				//bonus += 30 * skill_lv;
				//bonus += 5 * skill_lv; //custom habib
				bonus += sd->status.job_level * skill_lv; //custom habib
			if (pc_checkskill(sd,SU_SPRITEMABLE) > 0)
				bonus += 100;
			if (pc_checkskill(sd, SU_POWEROFSEA) > 0) {
				bonus += 100;
...
static unsigned int status_calc_maxhpsp_pc(map_session_data* sd, unsigned int stat, bool isHP) {
	...
	if (isHP) { //Calculates MaxHP
		...
		dmax = job->base_hp[level-1] * (1 + (umax(stat,1) * 0.02)); //custom +2% HP per vit
		...
		dmax = job->base_sp[level-1] * (1 + (umax(stat,1) * 0.025)); //custom +2.5% SP per INT
...
bool status_calc_weight(map_session_data *sd, enum e_status_calc_weight_opt flag)
{
	...
	sd->max_weight = job_db.get_maxWeight(pc_mapid2jobid(sd->class_, sd->status.sex)) + sd->status.str * 1500; //CUSTOM
	...
	for (i = 0; i < EQI_MAX; i++) {
		...
		if (sd->inventory_data[index]->type == IT_WEAPON) {
			...
			if(sd->inventory.u.items_inventory[index].card[0] == CARD0_FORGE) { // Forged weapon
				...
				if(wd->star >= 15) wd->star = 30; // 3 Star Crumbs now give +40 dmg
				//if(wd->star >= 3) wd->star = 8; // 3 Star Crumbs now give +8 dmg CUSTOM HABIB
...
void status_calc_regen(struct block_list *bl, struct status_data *status, struct regen_data *regen)
{
	...
	if( sd ) {
		...
		if( (skill=pc_checkskill(sd,WM_LESSON)) > 0 )
			//val += 3 + 3 * skill;
			//val += 3 * skill; //custom habib
			val += sd->status.job_level / 2 * skill; //custom habib
...
static unsigned short status_calc_speed(struct block_list *bl, status_change *sc, int speed)
{
	TBL_PC* sd = BL_CAST(BL_PC, bl);
	int speed_rate = 100;
	//md.damage = (int64)sstatus->hp + (atk.damage * (int64)sstatus->hp * skill_lv) / (int64)sstatus->max_hp; // custom habib test
	...
	if( sd && sd->ud.skilltimer != INVALID_TIMER && (pc_checkskill(sd,SA_FREECAST) > 0 || sd->ud.skill_id == LG_EXEEDBREAK) ) {
		...
		//CUSTOM HABIB FUNCTION START
		struct status_data *sstatus = status_get_status_data(bl); //custom habib test
		int bb_diff = 1;
		int incagi_diff = 1;
		//bb_diff = max((1 - (int64)sstatus->hp / (int64)sstatus->max_hp) * 30, 0);
		if( sd )
			bb_diff = (sstatus->agi + sd->status.job_level * 3) / 1.5;
		if( sd )
			incagi_diff = (5 + pc_checkskill(sd, NV_SLEIPNIR) + sd->status.job_level * 3);
		//bb_diff = (sstatus->agi);
		//CUSTOM HABIB FUNCTION END
		
		// GetMoveHasteValue1()
		if( sc->getSCE(SC_SPEEDUP1) ) // !FIXME: used both by NPC_AGIUP and Speed Potion script
			val = max( val, sc->getSCE(SC_SPEEDUP1)->val1 );
		if (pc_checkskill(sd, BB_CORE) > 0) //custom speed bb_core passive barbarian
			//val = max( val, 10 );
			val = max( val, bb_diff );
		if( sc->getSCE(SC_INCREASEAGI) )
			//val = max( val, 25 );
			val = max( val, incagi_diff );
		if( sc->getSCE(SC_WINDWALK) )
	...
	if( sd && pc_iscarton(sd) )
		...
		speed += speed * (50 - 10 * pc_checkskill(sd,MC_PUSHCART)) / 100; //habib
	if( sc->getSCE(SC_PARALYSE) && sc->getSCE(SC_PARALYSE)->val3 == 1 )
		speed += speed * 50 / 100;
	if( speed_rate != 100 )
...
static short status_calc_aspd(struct block_list *bl, status_change *sc, bool fixed)
{
	...
	if (fixed) {
		...
		if (sc->getSCE(SC_INCREASEAGI))
			...
			bonus += sc->getSCE(SC_INCREASEAGI)->val1*2;
...
t_tick status_get_sc_def(struct block_list *src, struct block_list *bl, enum sc_type type, int rate, t_tick tick, unsigned char flag)
{
	...
	switch (type) {
		...
		case SC_MARSHOFABYSS:
			...
			//custom habib skill_db.yml changes Duration1 to 5sec
...
//		if (util::vector_exists(status_get_race2(bl), RC2_GVG) && status_sc2scb_flag(type)&SCB_MAXHP) return 0;
	...
	switch(type)
	{
		...
		case SC_DECREASEAGI:
			...
			val2 = 3; // Agi change custom habib
			break;
		case SC_INCREASEAGI:
			if (type == SC_ADORAMUS) {
				// 1000% base chance to blind, but still can be resisted
		...
		case SC_BLEEDING:
		{
			clif_specialeffect(bl,1054,AREA); //custom visual effect habib EF_BURNING = 1054
		...
		case SC_HIDING:
			...
			val4 = val1; // Seconds before SP substraction happen.
			break;
		case SC_CHASEWALK:
		...
		case SC_PARRYING:
			...
			val2 = 30 + val1*14; // Block Chance custom habib
			break;
		...
		case SC_FRIGG_SONG:
			...
			val2 = 2 * val1; // maxhp bonus custom habib +2% * skill lv
			val3 = 1 + val1; // healing custom habib
			tick_time = 1000;
			val4 = tick / tick_time;
			break;
		case SC_FLASHCOMBO:
...
int status_change_end(struct block_list* bl, enum sc_type type, int tid)
{
	...
	switch(type) {
		...
		case SC_ENDURE:
		case SC_BLEEDING:
            clif_specialeffect_remove(bl, 1054, AREA, bl); //effect burning
		...
		case SC_WHITEIMPRISON:
			{
				...
				status_fix_damage(src,bl,8*sce->val1,clif_damage(bl,bl,gettick(),0,0,8*sce->val1,0,DMG_NORMAL,0,false),WL_WHITEIMPRISON);
			}
			break;
		case SC_WUGDASH:
			{
...
TIMER_FUNC(status_change_timer){
	...
	case SC_BLEEDING:
		if (sce->val4 >= 0) {
			//clif_specialeffect(bl,1054,AREA); NOW USED ON PREVIOUS CASE  //bl = target. or src = player. | 1 = effect ID or name | AREA = seen by everyone, can be changed to Player only if I remember correctly. //1054 = EF_BURNING
			//int64 damage = rnd() % 600 + 200;
			//clif_emotion(bl,ET_CRY); NOW USED ON PREVIOUS CASE
			int64 damage = 3 + (1 * status->max_hp) / 100;
	...
	case SC_BURNING:
		if (sce->val4 >= 0) {
			...
			int64 damage = 7 + (3 * status->max_hp) / 100; // Deals fixed (7 + 3%*MaxHP)
			map_freeblock_lock();
			dounlock = true;
			status_fix_damage(bl, bl, damage, clif_damage(bl, bl, tick, 0, 1, damage, 1, DMG_NORMAL, 0, false),0);
		}
...
static int status_natural_heal(struct block_list* bl, va_list args)
{
	...
	if (flag&(RGN_HP|RGN_SP)) {
		...
		if(vd && vd->dead_sit == 2)
			...
			multi += 5; //This causes the interval to be 5x faster //custom habib
		if(regen->state.gc)
			multi += 1; //This causes the interval to be halved
	}
