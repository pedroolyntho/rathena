...
static bool itemdb_read_sqldb_sub(std::vector<std::string> str) {
	...
	if (!str[++index].empty())
		jobs["Barbarian"] << (std::stoi(str[index]) ? "true" : "false");
	if (!str[++index].empty())
		jobs["Cleric"] << (std::stoi(str[index]) ? "true" : "false");
...
static int itemdb_read_sqldb(void) {
	...
	for( uint8 fi = 0; fi < ARRAYLENGTH(item_db_name); ++fi ) {
		...
		if( SQL_ERROR == Sql_Query(mmysql_handle, "SELECT `id`,`name_aegis`,`name_english`,`type`,`subtype`,`price_buy`,`price_sell`,`weight`,`attack`,`defense`,`range`,`slots`,"
			...
			"`job_monk`,`job_ninja`,`job_novice`,`job_priest`,`job_rogue`,`job_sage`,`job_soullinker`,`job_stargladiator`,`job_supernovice`,`job_swordman`,`job_taekwon`,`job_thief`,`job_wizard`,`job_barbarian`,"
