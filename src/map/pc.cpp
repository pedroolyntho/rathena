...
uint64 pc_jobid2mapid(unsigned short b_class)
{
	...
	//Custom Jobs
		case JOB_BARBARIAN:				return MAPID_BARBARIAN;
		case JOB_CLERIC:				return MAPID_CLERIC;
	//Unknown
		default:
			return -1;
	}
...
int pc_mapid2jobid(uint64 class_, int sex)
{
	...
	//4-2 Jobs
		case MAPID_BARBARIAN:             return JOB_BARBARIAN;
		case MAPID_CLERIC:                return JOB_CLERIC;
	//Unknown
		default:
			return -1;
	}
...
const char* job_name(int class_)
{
	...
	//Custom Job
	case JOB_BARBARIAN:
		return msg_txt(NULL,1800);
	case JOB_CLERIC:
		return msg_txt(NULL,1801);
...
// old formula #define PC_STATUS_POINT_COST(low) (((low) < 100) ? (2 + ((low) - 1) / 10) : (16 + 4 * (((low) - 100) / 5)))
#define PC_STATUS_POINT_COST(low) (((low) < 8) ? (1 + ((low) - 1) / 1) : (3 + (((low) - 1) / 1))) //a cada atributo gasto, aumenta 1 de custo, após o 8, aumenta 3 por atributo)
#else
