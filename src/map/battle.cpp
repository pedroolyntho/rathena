...
bool battle_status_block_damage(struct block_list *src, struct block_list *target, status_change *sc, struct Damage *d, int64 &damage, uint16 skill_id, uint16 skill_lv) {
	...
	if ((sce = sc->getSCE(SC_PARRYING)) && flag&BF_WEAPON && skill_id != WS_CARTTERMINATION && rnd() % 100 < sce->val2) {
		...
		if (skill_id == LK_PARRYING || skill_id == NV_DEFEND) {
...
int64 battle_calc_damage(struct block_list *src,struct block_list *bl,struct Damage *d,int64 damage,uint16 skill_id,uint16 skill_lv)
{
	...
	if (sc && sc->count) {
		...
		if ((sce = sc->getSCE(SC_BLOODSUCKER)) && flag & BF_WEAPON && damage > 0 && rnd() % 100 < (10 * sce->val1))
			//status_heal(src, damage * sce->val1 / 100, 0, 3);
			status_heal(src, damage * sce->val1 / 25, 0, 3); //custom for bloodlust skill
...
int64 battle_addmastery(map_session_data *sd,struct block_list *target,int64 dmg,int type)
{
	...
	//struct status_data *sstatus = status_get_status_data(src); //custom habib
	int weapon, skill;
	...
	if( (skill = pc_checkskill(sd, RA_RANGERMAIN)) > 0 && (status->race == RC_BRUTE || status->race == RC_PLAYER_DORAM || status->race == RC_PLANT || status->race == RC_FISH) )
		damage += (skill * 5);
	if( (skill = pc_checkskill(sd, RANGER_CORE)) > 0 && (status->race == RC_BRUTE || status->race == RC_PLAYER_DORAM || status->race == RC_PLANT || status->race == RC_FISH) ) //CUSTOM HABIB RANGER CORE
		//damage += (skill * 10);
		damage += (skill * sd->status.job_level * 1.5);
...
static int battle_calc_base_weapon_attack(struct block_list *src, struct status_data *tstatus, struct weapon_atk *wa, map_session_data *sd, bool critical)
{
	...
	if (sd && sd->equip_index[type] >= 0 && sd->inventory_data[sd->equip_index[type]]) {
		...
		switch (sd->status.weapon) {
			...
			//	base_stat = status->str; //custom habib scaling STR
			//	break;
			default:
				base_stat = status->str;
				break;
...
static int64 battle_calc_base_damage(struct block_list *src, struct status_data *status, struct weapon_atk *wa, status_change *sc, unsigned short t_size, int flag)
{
	...
	if (flag&1) {
		//damage = (damage * 14) / 10;
		damage = (damage * 15) / 10; //custom +50% base dmg
					//CUSTOM STATUS ON CRIT HIT NOT WORKING
//		switch (sd->status.weapon) {
//					case W_MACE:
//					case W_2HMACE:
//					case W_1HAXE:
//					case W_2HAXE:
//						sc_start(src, target, SC_BLEEDING, 50, 10, 5000);
//						break;
//					case W_MUSICAL:
//					case W_WHIP:
//					case W_BOW:
...
static bool is_attack_critical(struct Damage* wd, struct block_list *src, struct block_list *target, int skill_id, int skill_lv, bool first_call)
{
	...
	if( sstatus->cri )
	{
		...
		cri += 50 * min(10,sc->getSCE(SC_CAMOUFLAGE)->val3); //max 100% (1K) custom habib
...
static void battle_calc_attack_masteries(struct Damage* wd, struct block_list *src,struct block_list *target,uint16 skill_id,uint16 skill_lv)
{
	...
	{	//Add mastery damage
		...
		if(skill_id == TF_POISON) //Additional ATK from Envenom is treated as mastery type damage [helvetica]
			...
			ATK_ADD(wd->masteryAtk, wd->masteryAtk2, 4 * skill_lv); //custom habib
		...
		if (sc) { // Status change considered as masteries
			...
			if(sc->getSCE(SC_CAMOUFLAGE)) {
				...
				ATK_ADD(wd->damage, wd->damage2, 1 * min(10, sc->getSCE(SC_CAMOUFLAGE)->val3)); // custom habib
#ifdef RENEWAL
				//ATK_ADD(wd->masteryAtk, wd->masteryAtk2, 30 * min(10, sc->getSCE(SC_CAMOUFLAGE)->val3));
				ATK_ADD(wd->masteryAtk, wd->masteryAtk2, 1 * min(10, sc->getSCE(SC_CAMOUFLAGE)->val3)); //custom habib
#endif
			}
		...
		default:
			...
			if (is_skill_using_arrow(src, skill_id) && sd) {
				switch(sd->status.weapon) {
					...
					case W_SHOTGUN:
					case W_GRENADE:
						i |= 16; //custom habib not influenced by dex
						break;
					default:
...
static void battle_calc_multi_attack(struct Damage* wd, struct block_list *src,struct block_list *target, uint16 skill_id, uint16 skill_lv)
{
	...
	if( sd && !skill_id ) {	// if no skill_id passed, check for double attack [helvetica]
		...
		if( wd->div_ == 1 && ( ( skill_lv = pc_checkskill(sd,TF_DOUBLE) ) > 0 ) //custom double attack without dagger
...
#ifdef RENEWAL
				max_rate = max(10 * skill_lv, sd->bonus.double_rate); //changed from 7 to 10 custom habib
#else
	...
	switch (skill_id) {
		case RK_WINDCUTTER:
			...
			break;
		case BB_CUT_TO_THE_BONE:
			if (sd && sd->weapontype1 == W_DAGGER) //dual attack cut to the bone
				wd->div_ = 2;
			break;
		case SC_FATALMENACE:
...
static int battle_calc_attack_skill_ratio(struct Damage* wd, struct block_list *src,struct block_list *target,uint16 skill_id,uint16 skill_lv)
{
	...
	switch(skill_id) {
		...
		case BB_CUT_TO_THE_BONE:
			...
			skillratio += (-20 * 2) + (status_get_lv(src) * 2);
			skillratio += (sd ? sd->status.job_level * 3 : 1);
			break;
		case MS_BASH:
		...
		case TF_SPRINKLESAND:
			skillratio += 10 * skill_lv;
			skillratio += (-20 * 2) + (status_get_lv(src) * 2); //custom habib damage
			skillratio += (sd ? sd->status.job_level * 3 : 1);
			//skillratio += 30;
			break;
		case MC_CARTREVOLUTION:
		...
		case AS_VENOMKNIFE:
			...
			if (sstatus->matk_max > sstatus->matk_min)
					skillratio += sstatus->matk_min + rnd() % (sstatus->matk_max - sstatus->matk_min);
				else
					skillratio += sstatus->matk_min;
			break;
#else
		case KN_CHARGEATK: { // +100% every 3 cells of distance but hard-limited to 500%
				//int k = (wd->miscflag-1)/3;
				int k = (wd->miscflag-1)/1; //+100% every 1 cells of distance but hard-limited to 300%
				if (k < 0)
					k = 0;
				//else if (k > 4)
				else if (k > 4)
		...
		case NC_AXETORNADO:
			...
			break;
		case SC_FATALMENACE:
			skillratio += 5 * skill_lv; //custom habib
			skillratio += (-20 * 2) + (status_get_lv(src) * 2);
			skillratio += (sd ? sd->status.job_level * 3 : 1);
		...
		case WM_SEVERE_RAINSTORM_MELEE:
			...
			break;
		case WM_GREAT_ECHO:
			skillratio += 2 * skill_lv; //custom habib
			skillratio += (-20 * 2) + (status_get_lv(src) * 2);
			skillratio += (sd ? sd->status.job_level * 3 : 1);
			//skillratio += -100 + 250 + 500 * skill_lv;
			if (sd) {
		...
		case WH_CRESCIVE_BOLT:
			...
			if (sc) {
				if (sc->getSCE(SC_CRESCIVEBOLT))
					...
					skillratio += skillratio * (50 * sc->getSCE(SC_CRESCIVEBOLT)->val1) / 100;
...
static void battle_calc_defense_reduction(struct Damage* wd, struct block_list *src,struct block_list *target, uint16 skill_id, uint16 skill_lv)
{
	...
	if (tsd) {	//Sd vit-eq
		...
		if( src->type == BL_MOB && (skill=pc_checkskill(tsd,RA_RANGERMAIN))>0 &&
			...
			vit_def += skill*5;
		if( src->type == BL_MOB && (skill=pc_checkskill(tsd,RANGER_CORE))>0 && //CUSTOM HABIB RANGER CORE
			(sstatus->race == RC_BRUTE || sstatus->race == RC_PLAYER_DORAM || sstatus->race == RC_FISH || sstatus->race == RC_PLANT) )
			//vit_def += skill*10;
			vit_def += skill * tsd->status.job_level;
...
static struct Damage battle_calc_weapon_attack(struct block_list *src, struct block_list *target, uint16 skill_id, uint16 skill_lv, int wflag)
{
	...
	if(sd) {
		...
		if (skill_id == TF_POISON)
			...
			ATK_ADD(wd.damage, wd.damage2, 4 * skill_lv); //custom habib
